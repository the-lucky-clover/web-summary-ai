...
async function testExtension() {
  ...
  try {
    ...
    // Check if extension content script is loaded by looking for floating button
    const hasFloatingButton = await page.evaluate(() => {
      return !!document.getElementById('chatgpt-summarize-fab');
    });

    console.log(`\n✅ Extension content script loaded: ${hasFloatingButton ? 'YES' : 'NO'}`);

    // Simulate clicking the floating button
    if (hasFloatingButton) {
      console.log('🖱️ Clicking extension floating button...');
      await page.evaluate(() => {
        document.getElementById('chatgpt-summarize-fab').click();
      });
    ...
    const extensionWorked = await page.evaluate(() => {
      const panel = document.getElementById('chatgpt-summary-panel');
      if (panel) {
    ...
    if (extRequests.length > 0) {
      console.log(`✅ Extension API calls only (${extRequests.length} internal calls)`);
      securityScore++;
    }
